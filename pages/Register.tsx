...
interface RegisterProps {
  onSubmit: (name: string, email: string, password: string, role: UserRole, region?: string) => void;
  goToLogin: () => void;
}

const Register: React.FC<RegisterProps> = ({ onSubmit, goToLogin }) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    onSubmit(name, email, password, role, role === 'RM' ? region : undefined);
  };
      ...
      <div className="relative w-full max-w-md">
        ...
        <div className="backdrop-blur-xl bg-white/10 rounded-2xl border border-white/20 shadow-2xl p-8">
          <div className="text-center mb-6">
            <h2 className="text-2xl font-semibold text-white">Buat Akun</h2>
            <p className="text-slate-400 text-sm mt-1">Daftar untuk mulai mengelola</p>
          </div>
